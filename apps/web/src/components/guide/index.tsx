...
import { CUSTOM_ELEMENT_CLASS } from '../../constant';
import { IGuide } from '../../utils/guide/typings';
...
const Guide: React.FC<IGuide> = (props) => {
  ...
  const visible = Object.prototype.hasOwnProperty.call(props, 'visible') ? props.visible : true;
   ...
   */
  const realWindow = useMemo(() => (anchorEl ? getWindow(anchorEl) : null), [anchorEl]);

  const realDocument = useMemo(() => (anchorEl ? getDocumentElement(anchorEl as Element) : null), [anchorEl]);
  ...
  useEffect(() => {
    if (visible) {
      ...
      const expireDateParse = new Date(Date.parse(expireDate.replace(/-/g, '/')));
  ...
  useEffect((): (() => void) | void => {
    if (realWindow && realDocument) {
      ...
      return () => {
        realWindow.removeEventListener('keydown', handleKeydown as EventListener);
      };
    }
  ...
  return (!mask || initOverflowVal) && parentEl ? (
    <>
      {mask && <Mask className={maskClassName} anchorEl={anchorEl as Element} realWindow={realWindow as Window} />}
      <Modal
